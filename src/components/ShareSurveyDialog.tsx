  ...
  if (showFullScreenQR) {
    return (
      <Dialog open={showFullScreenQR} onOpenChange={setShowFullScreenQR}>
        <DialogContent className="w-[90vw] max-w-[280px] sm:max-w-xs mx-auto p-2 sm:p-4 overflow-hidden">
          <DialogHeader className="pb-2">
            <DialogTitle className="text-center text-sm sm:text-base font-bold truncate">
              {t('share.scanToAccess', 'Scanner pour accéder')}
            </DialogTitle>
            <DialogDescription className="text-center text-[10px] sm:text-xs line-clamp-1 px-1">
              {surveyTitle}
            </DialogDescription>
          </DialogHeader>
          
          <div className="flex flex-col items-center py-2" ref={fullScreenQrRef}>
            <div className="bg-background p-2 sm:p-4 rounded-xl shadow-lg border border-border">
              <QRCodeCanvas
                value={surveyUrl}
          ...
          <div className="grid grid-cols-2 gap-2">
            <Button onClick={() => handleDownloadQR(fullScreenQrRef)} variant="outline" size="sm" className="text-[10px] sm:text-xs h-7 sm:h-8 px-2">
              <Download className="h-3 w-3 mr-0.5 sm:mr-1 flex-shrink-0" />
              {t('share.download', 'Télécharger')}
            </Button>
            <Button onClick={() => handleShareQR(fullScreenQrRef)} size="sm" className="text-[10px] sm:text-xs h-7 sm:h-8 px-2">
          ...
          <Button 
            ...
            className="w-full mt-1 text-[10px] sm:text-xs h-6 sm:h-7"
          >
    ...
    <Dialog open={mainDialogOpen} onOpenChange={setMainDialogOpen}>
      ...
      <DialogContent className="w-[90vw] max-w-[300px] sm:max-w-sm mx-auto p-2 sm:p-3 max-h-[85vh] overflow-y-auto overflow-x-hidden">
        <DialogHeader className="pb-1 sm:pb-2">
          <DialogTitle className="flex items-center gap-1 sm:gap-2 text-xs sm:text-sm">
            <Link2 className="h-3 w-3 sm:h-4 sm:w-4 text-primary flex-shrink-0" />
            <span className="truncate text-xs sm:text-sm">{t('share.shareSurvey', "Partager l'enquête")}</span>
          </DialogTitle>
        ...
        <div className="space-y-2 py-1 sm:py-2">
          ...
          {coverImageUrl && (
            <div className="rounded-lg overflow-hidden border border-border bg-muted hidden sm:block">
              <div className="flex items-center gap-2 p-1.5 sm:p-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-[10px] sm:text-xs truncate text-foreground">{surveyTitle}</p>
          ...
          <div className="flex items-center gap-1.5">
            <Input 
              value={surveyUrl} 
              readOnly 
              className="font-mono text-[8px] sm:text-[9px] flex-1 min-w-0 h-7 sm:h-8 px-1.5 sm:px-2" 
            />
            <Button size="icon" variant="outline" onClick={handleCopy} className="flex-shrink-0 h-7 w-7 sm:h-8 sm:w-8">
              {copied ? (
          ...
          {/* Action Buttons */}
          <div className="grid grid-cols-2 gap-2">
            <Button onClick={handleShare} size="sm" className="text-[10px] sm:text-xs h-7 sm:h-8">
              <Share2 className="h-3 w-3 mr-0.5 sm:mr-1 flex-shrink-0" />
              {t('share.share', 'Partager')}
            </Button>
            <Button variant="outline" onClick={() => window.open(surveyUrl, '_blank')} size="sm" className="text-[10px] sm:text-xs h-7 sm:h-8">
              <ExternalLink className="h-3 w-3 mr-0.5 sm:mr-1 flex-shrink-0" />
              {t('share.open', 'Ouvrir')}
            </Button>
          </div>
          ...
          <div className="rounded-lg border border-border bg-muted/30 p-1.5 sm:p-2">
            <div className="flex items-center justify-between mb-1.5 sm:mb-2">
              <p className="text-[10px] sm:text-xs font-medium text-foreground">QR Code</p>
              <Button 
                variant="ghost" 
                size="sm" 
                onClick={() => {
                ...
                className="text-[9px] sm:text-[10px] h-5 sm:h-6 px-1 sm:px-1.5"
              >
                <Maximize2 className="h-2.5 w-2.5 sm:h-3 sm:w-3 mr-0.5" />
                {t('share.fullscreen', 'Agrandir')}
              </Button>
            </div>
            <div className="flex items-center gap-1.5 sm:gap-2" ref={qrRef}>
              <div className="bg-background p-1 sm:p-1.5 rounded-lg shadow-sm border border-border flex-shrink-0">
                <QRCodeCanvas
                  value={surveyUrl}
                  size={60}
                  includeMargin
              ...
              <div className="flex flex-col gap-1 flex-1 min-w-0">
                <Button 
                  ...
                  variant="outline" 
                  size="sm" 
                  className="w-full text-[9px] sm:text-[10px] h-6 sm:h-7"
                >
                  <Download className="h-2.5 w-2.5 sm:h-3 sm:w-3 mr-0.5 sm:mr-1 flex-shrink-0" />
                  {t('share.download', 'Télécharger')}
                </Button>
                <Button 
                  ...
                  className="w-full text-[9px] sm:text-[10px] h-6 sm:h-7"
                >
                  <Share2 className="h-2.5 w-2.5 sm:h-3 sm:w-3 mr-0.5 sm:mr-1 flex-shrink-0" />
