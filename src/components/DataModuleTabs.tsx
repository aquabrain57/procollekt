...
import { Table2, BarChart3, Crown, Activity } from 'lucide-react';
import { DbSurvey, DbSurveyResponse, useSurveyResponses } from '@/hooks/useSurveys';
import { ResponsesTable } from '@/components/ResponsesTable';
import { AnalyticsDashboard } from '@/components/analytics';
import { PremiumReport } from '@/components/PremiumReport';
import { RealtimeDashboard } from '@/components/RealtimeDashboard';
import { DataModuleHeader } from '@/components/DataModuleHeader';
...
export const DataModuleTabs = ({ survey, responses }: DataModuleTabsProps) => {
  const { deleteAllResponses, refetch } = useSurveyResponses(survey.id);

  const handleDeleteAllResponses = async () => {
    const success = await deleteAllResponses(survey.id);
    if (success) {
      refetch();
    }
    return success;
